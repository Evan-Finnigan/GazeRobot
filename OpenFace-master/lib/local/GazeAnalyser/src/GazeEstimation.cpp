...
void GazeAnalysis::EstimateGaze(const LandmarkDetector::CLNF& clnf_model, cv::Point3f& gaze_absolute, float fx, float fy, float cx, float cy, bool left_eye)
{
	...
	if (part == -1)
	{
		std::cout << "Couldn't find the eye model, something is wrong" << std::endl;
...
// overloaded to allow for getting the eyeball center as well as the gaze direction (gaze_absolute)
void GazeAnalysis::EstimateGaze(const LandmarkDetector::CLNF& clnf_model, cv::Point3f& gaze_absolute, cv::Point3f& eyeballCentre, float fx, float fy, float cx, float cy, bool left_eye)
{
	cv::Vec6f headPose = LandmarkDetector::GetPose(clnf_model, fx, fy, cx, cy);
	cv::Vec3f eulerAngles(headPose(3), headPose(4), headPose(5));
	cv::Matx33f rotMat = Utilities::Euler2RotationMatrix(eulerAngles);
...
// overloaded to allow for getting the eyeball center as well as the gaze direction (gaze_absolute)
void GazeAnalysis::EstimateGaze(const LandmarkDetector::CLNF& clnf_model, cv::Point2f pupilLoc2d, cv::Point3f& gaze_absolute, cv::Point3f& eyeballCentre, float fx, float fy, float cx, float cy, bool left_eye)
{
	cv::Mat K = (cv::Mat_<float>(3,3) << fx, 0, cx, 0, fy, cy, 0, 0, 1);
	cv::Mat p = K.inv() *  (cv::Mat_<float>(3,1) << pupilLoc2d.x, pupilLoc2d.y, 1);
	cv::Point3f pupil(p.at<float>(0, 0), p.at<float>(1, 0), p.at<float>(2, 0));
	...
	cv::Point3f d = eyeballCentre - rayDir.dot(eyeballCentre) / rayDir.dot(rayDir) *  rayDir;
