...
namespace GazeAnalysis
{
	...
	//overloaded to allow getting the eyeball centers
	void EstimateGaze(const LandmarkDetector::CLNF& clnf_model, cv::Point3f& gaze_absolute, cv::Point3f& eyeballCentre, float fx, float fy, float cx, float cy, bool left_eye);
	// another EstimateGaze that can use the 2d pupil location to get gaze direction
	void EstimateGaze(const LandmarkDetector::CLNF& clnf_model, cv::Point2f pupilLoc2d, cv::Point3f& gaze_absolute, cv::Point3f& eyeballCentre, float fx, float fy, float cx, float cy, bool left_eye);
